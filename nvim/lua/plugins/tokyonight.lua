...
--
-- ── bschur_custom theme ─────────────────────────────────────────────────────
-- Palette derived from the Snowflake documentation site's DARK mode
-- (docs.snowflake.com). Cool near-black backgrounds, blue-gray neutrals, a
-- royal-blue primary accent (#1a6ce7 / #5999f8) and a brand cyan (#29b5e8).
-- Layered onto tokyonight's `storm` engine via `on_colors` so all of
-- tokyonight's derived colors (diffs, borders, terminal, visual) stay coherent.
--
...
-- bschur_custom dark-mode source colors (derived from docs.snowflake.com).
local bschur_custom = {
